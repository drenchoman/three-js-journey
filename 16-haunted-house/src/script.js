...
// Fog
const fog = new THREE.Fog('#262387', 1, 15);
scene.fog = fog;
...
const doorRoughnessexture = textureLoader.load(
...
bush4.position.set(-0.1, 0.05, 2.6);
...
for (let i = 0; i < 50; i++) {
  ...
  grave.rotation.z = Math.random() - 0.4;
...
// Directional light
const moonLight = new THREE.DirectionalLight('#b9df55', 0.26);
...
const tick = () => {
  ...
  const ghostAngle2 = -elapsedTime * 0.15;
  ...
  ghost3.position.x =
    Math.cos(ghostAngle3) * (7 + Math.sin(elapsedTime * 0.42));
  ghost3.position.z =
    Math.sin(ghostAngle3) * (7 + Math.sin(elapsedTime * 0.45));
  ghost3.position.y =
